...
| 18 | `webkit-bubblewrap-sfos-sandbox.patch` | 1 | `ATLANTIC_ENABLE_SANDBOX`<br>`WEBKIT_SWIFT_STDLIB_LIBRARY_PATH` | Re-enable the WPE bubblewrap process sandbox on SFOS / Android-4.14: --dev-bind / / (no pivot_root and no --dev masking of the GPU nodes), a shared network namespace for the Web and GPU processes (hyb… |
...
| 38 | `webkit-pointerdown-cancel-keeps-click.patch` | 2 | — | Canceling pointerdown must not eat the tap's click. |
